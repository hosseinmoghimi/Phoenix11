...
class ResumeIndex(models.Model,LinkHelper):
    ...
    def __str__(self):
        return self.language+" : "+(self.person.name if self.title is None else self.title)
    ...
    def image(self):
        if self.image_main_origin:
            ...
            return f'{STATIC_URL}{TEMPLATE_ROOT}/img/person-img.jpg'
...
class ResumePortfolio(ResumePage):
    resume_index=models.ForeignKey("resumeindex", verbose_name=_("resume"), on_delete=models.CASCADE)
    filter=models.CharField(_("filter"), max_length=50)
...
class ResumeSkill(models.Model):
    ...
    def __str__(self):
        return f"""{self.resume_index.person.name} : {self.title} : {self.percentage}"""
...
class ResumeFact(models.Model):
    ...
    def __str__(self):
        return f"""{self.resume_index.person.name} : {self.title} : {self.count}"""
...
class ContactMessage(models.Model):
    ...
    def __str__(self):
        return f"""{self.resume_index.person.name} : @{self.full_name}"""
