...
from .repo import TableRepo,MenuRepo,DeskRepo
from .serializers import TableSerializer,MenuSerializer,DeskSerializer
...
class AddDeskApi(APIView):
    def post(self,request,*args, **kwargs):
        ...
        from utility.message import INVALID_FORM_VALUE_MESSAGE
        message=INVALID_FORM_VALUE_MESSAGE
        add_desk_form=AddDeskForm(request.POST)
        if add_desk_form.is_valid():
            log=333
            cd=add_desk_form.cleaned_data
            result,message,desk=DeskRepo(request=request).add_desk(**cd)
            if desk is not None:
                context['desk']=DeskSerializer(desk).data
        context['message']=message
        context['result']=result
        context['log']=log
        return JsonResponse(context)
