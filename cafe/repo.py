from .models import Table,Menu,MenuItem,DeskCustomer,Desk
...
class DeskRepo():
    ...
    def add_desk(self,*args,**kwargs):
        ...
        if 'title' in kwargs:
            ...
            old=Desk.objects.filter(title=kwargs['title']).first()
            if old is not None:
                message='عنوان تکراری برای میز جدید'
                return FAILED,message,None
            
        if 'desk_no' in kwargs:
            if kwargs["desk_no"]>0:
                desk.desk_no=kwargs["desk_no"]
        if 'color' in kwargs:
        ...
        if 'code' in kwargs:
            desk.code=kwargs["code"]
            old=Desk.objects.filter(code=kwargs['code']).first()
            if old is not None:
                message='کد تکراری برای میز جدید'
                return FAILED,message,None
